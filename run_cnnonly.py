...
from layers_1 import Relu, Softmax, Linear, Convolution,  ConvolutionSquare, Pooling, PoolingSquare, Batchnorm,Dropout,RNN4D
...
#model.add(RNN4D('rnn1', batch_size, 128, 16, 10, 0.1))		#	HIDDEN_DIM, INPUT_DIM, 0.1
#model.add(LSTM4D('rnn1', batch_size, 128, 32, 10 ,0.1, True))

model.add(Linear('fc7',128*10*17 , 2048, 0.005))
model.add(Batchnorm('bn7'))
model.add(Relu('relu7'))  
model.add(Dropout('dropout1'))
model.add(Linear('fc8',2048 , 2048, 0.005))
model.add(Batchnorm('bn8'))
model.add(Relu('relu8'))  
model.add(Dropout('dropout2'))
model.add(Linear('fc9', 2048, 4, 0.005))
...
solve_net(model, optim, train_data, train_label, test_data, test_label,
          batch_size=batch_size, max_epoch=100, disp_freq=250, test_freq=2000, half_step = 20, network_name = name)
#batch <30
