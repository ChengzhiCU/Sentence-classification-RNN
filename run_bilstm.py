...
from layers_1 import Relu, Softmax, Linear, Convolution,  ConvolutionSquare, Pooling, PoolingSquare, Batchnorm,Dropout,RNN4D,LSTM4D,LSTM4D_back
...
model.add(LSTM4D('rnn1', batch_size, 128, 32, 10 ,0.1))
model.add(LSTM4D_back('rnn1', batch_size, 128, 32, 32, 0.1))		#	HIDDEN_DIM, INPUT_DIM, 0.1


model.add(Linear('fc7',128*32*17, 2048, 0.005))
...
solve_net(model, optim, train_data, train_label, test_data, test_label,
          batch_size=batch_size, max_epoch=100, disp_freq=250, test_freq=500, half_step= 25, network_name = name)
#batch <30
